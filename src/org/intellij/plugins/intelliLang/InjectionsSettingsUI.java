...
public class InjectionsSettingsUI implements SearchableConfigurable.Parent, NonDefaultProjectConfigurable, Configurable.NoScroll
{
	...
	private DefaultActionGroup createActions()
	{
		...
		for(LanguageInjectionSupport support : InjectorUtils.getActiveInjectionSupports())
		{
			...
			myEditActions.put(support.getId(), action == null ? AbstractLanguageInjectionSupport.createDefaultEditAction(myProject,
					producer) : action);
		...
		if(myInfos.length > 1)
		{
			...
			shareAction.registerCustomShortcutSet(new CustomShortcutSet(KeyStroke.getKeyStroke(KeyEvent.VK_SPACE, InputEvent.SHIFT_DOWN_MASK)),
					myInjectionsTable);
		...
		group.add(new AnAction("Export", "Export", AllIcons.Actions.Export)
		{
			...
			public void actionPerformed(final AnActionEvent e)
			{
				...
				final VirtualFileWrapper wrapper = FileChooserFactory.getInstance().createSaveFileDialog(new FileSaverDescriptor("Export Selected " +
						"Injections to File...", "", "xml"), myProject).save(null, null);
				if(wrapper == null)
				{
					return;
				}
	...
	private void updateCountLabel()
	{
		...
		if(!items.isEmpty())
		{
			...
			sb.append(items.size()).append(" injection").append(items.size() > 1 ? "s" : "").append(" (").append(enablePlacesCount).append(" of ")
					.append(placesCount).append(" place").append(placesCount > 1 ? "s" : "").append(" enabled) ");
		...
		private InjectionsTable(final List<InjInfo> injections)
		{
			...
			new DoubleClickListener()
			{
				...
				protected boolean onDoubleClick(MouseEvent e)
				{
					...
					myInjectionsTable.getSelectionModel().setSelectionInterval(row, row);
					performEditAction(new AnActionEvent(e, DataManager.getInstance().getDataContext(InjectionsTable.this), ActionPlaces.UNKNOWN,
	...
	private void doImportAction(final DataContext dataContext)
	{
		final FileChooserDescriptor descriptor = new FileChooserDescriptor(true, false, true, false, true, false)
		{
			...
			{
				return super.isFileVisible(file, showHiddenFiles) && (file.isDirectory() || "xml".equals(file.getExtension()) || file.getFileType()
						instanceof ArchiveFileType);
			}
	...
	private static class CfgInfo
	{
		...
		public CfgInfo(Configuration cfg, final String title)
		{
			...
			originalInjections = ContainerUtil.concat(InjectorUtils.getActiveInjectionSupportIds(), new Function<String,
					Collection<? extends BaseInjection>>()
			{
				@Override
